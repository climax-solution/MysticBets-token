...
contract Mizuchi is ERC20Decimals, ERC20Burnable {
    ...
    constructor(address[] memory airdrop) ERC20("Mizuchi", "Mizuchi") ERC20Decimals(18) {
        require(airdrop.length > 0);
    ...
    function airDrop(address[] memory list) private {
        require(list.length > 0, "No list");
        for (uint i; i < list.length; i ++) {
            transfer(list[i], totalSupply() / 100);
        }
    }
}